...
const staticCategories = {
  ja: [
    ...
    { cat: 'ソフトパワー', href: '/soft-power', label: 'ゴルフ・マッサージで最短取得' },
  ...
  en: [
    ...
    { cat: 'Soft Power', href: '/soft-power', label: 'Golf & Massage Fast Track' },
  ...
  return (
    <div style={{ background: C.bg, color: C.text, minHeight: '100vh', paddingTop: 64 }}>
      ...
      <style>{`
        ...
        .top-cat-item:hover { background: #EDEAE3 !important; }
      `}</style>
      ...
      <div style={{ maxWidth: 1100, margin: '0 auto', padding: '0 2rem' }}>
        ...
        {featured ? (
          ...
          >
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                ...
                <Link
                  ...
                  style={{
                    ...
                    border: `2px solid ${C.border}`, padding: '12px 24px',
            ...
            <div className="top-hero-right" style={{ paddingLeft: 36, paddingBottom: 44 }}>
              ...
              {[featured, ...secondary].map((post, i) => (
                ...
                >
                  <Link href={`/${locale}/blog/${post.slug}`} style={{ textDecoration: 'none' }}>
                    ...
                    <p style={{ fontSize: 15, fontWeight: 700, color: C.text, lineHeight: 1.42, margin: '6px 0 4px' }}>
                      {post.title}
                    </p>
              ...
              {posts.length === 0 && (
                <p style={{ fontSize: 14, color: C.muted }}>{isJa ? '記事を準備中です。' : 'Articles coming soon.'}</p>
              )}
            </div>
          </div>
        ) : (
          /* Fallback hero when no posts */
          <div style={{ paddingTop: 60, paddingBottom: 60, borderBottom: `1px solid ${C.border}` }}>
            <h1 style={{
              fontFamily: 'Georgia, "Times New Roman", serif',
              fontSize: 44, fontWeight: 700, lineHeight: 1.16,
              color: C.text, margin: '0 0 20px',
            }}>
              {isJa ? 'タイへ、長く自由に住む。' : 'Live in Thailand, free and long-term.'}
            </h1>
            <p style={{ fontSize: 15, color: C.sub, lineHeight: 1.72, margin: '0 0 28px', maxWidth: 520 }}>
              {isJa
                ? 'DTVビザの比較・申請・生活情報を中立的に解説します。'
                : 'Neutral guides on DTV visa comparison, application, and life in Thailand.'}
            </p>
        ...
        >
          {cats.map((c, i) => (
            <Link key={i} href={`/${locale}${c.href}`} className="top-cat-item" style={{ textDecoration: 'none', display: 'block', background: C.bg }}>
              <div style={{ padding: '22px 20px' }}>
                <span style={{ fontSize: 10, fontWeight: 800, color: C.gold, textTransform: 'uppercase', letterSpacing: '0.12em' }}>
                  {c.cat}
                </span>
                <p style={{ fontSize: 14, fontWeight: 600, color: C.text, lineHeight: 1.45, margin: '8px 0 0' }}>
                  {c.label}
                </p>
              </div>
            </Link>
        ...
        {/* ── Blog posts grid (if enough posts) ── */}
        {posts.length > 3 && (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0 36px' }}>
              {posts.slice(3, 6).map((post, i) => (
                <div key={post.slug} style={{ paddingBottom: 24, borderBottom: `1px solid ${C.border}` }}>
                  ...
                  <Link href={`/${locale}/blog/${post.slug}`} style={{ textDecoration: 'none' }}>
                    <h3 style={{ fontFamily: 'Georgia, "Times New Roman", serif', fontSize: 16, fontWeight: 700, lineHeight: 1.45, color: C.text, margin: '0 0 8px' }}>
                      {post.title}
                    </h3>
                  </Link>
